...
;;  2 5/3/95   slh  bill's version, works with 3.0. other version renamed
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; This version uses the new general termination support
;;;
    ...
    (loop
      (without-interrupts
       ...
       (setf cell (car list)
             ...
             (cdr list) nil))
      ;; bad, but dead macptrs appear in the population after restart.
      ;; Is it an MCL bug? The test below ignores such dead macptrs. Hernan.
      (unless (and (macptrp (car cell)) (funcall (cdr cell) (car cell)))
        (without-interrupts
         (setf (cdr list) (population-data population)
               (population-data population) list))))))

(defun macptr->dead-macptr (macptr)
  (require-type macptr 'macptr)
  (%macptr->dead-macptr macptr))
...
;;;;; end of new code for MCLs with support for trminable macptrs


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; The old version needed to work harder.
;;; It also requires a kernel patch.
;;; It won't work in a vanilla MCL 3.0 (or 2.0).
;;;

;; This was required before MCL3.1 and MCL4.0 which include some supprt for terminable macptrs
;; Now here for documentation purposes
#+ignore
(progn
...
(defun cons-terminable-macptr (macptr &optional (id 0))
  ...
  (let ((p (make-uvector 4 $v_macptr :initial-element 0)))
...
#| This is for MCL 2.x  -- We'll no longer try to keep this source working with that
(defun macptr->dead-macptr (macptrObject)
  (require-type macptrObject 'macptr)
  (lap
    (move.l (varg macptrObject) atemp0)
...
;;; Clear the macptr termination data structure before save application. 
...
#|
	...
	4  	 9/ 4/96	Hernan  	Added latest changes from MCL 4.0
	5  	10/22/96	sidney  	make it work in MCL 3.1 too
	6  	 2/27/97	Hernan  	
