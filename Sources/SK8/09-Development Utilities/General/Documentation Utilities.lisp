...
(defparameter *doc-window-font* '("Geneva" 9 :plain))
...
;;; Here is a class for editing SK8Script...

(defclass SK8Script-fred-window (fred-window)
  ())

(defmethod initialize-instance ((w SK8Script-fred-window) &rest initargs &key (window-show t))
  (apply #'call-next-method w :window-show nil initargs)
  (setf (slot-value (window-key-handler w) 'comtab) sk8dev::*script-fred-comtab*)
  (setf (fr.tabcount (frec (window-key-handler w))) 3)
  (fred-update w)                       ; otherwise window-title flashes
  (when window-show
    (window-show w)))

(defmacro making-SK8Script-fred-window (&body body)
  `(let ((*default-editor-class* 'SK8Script-fred-window))
     ,@body))
...
(defun insert-object-template (w)
  (let* ((theBuf (fred-buffer w))
    ...
    (ed-insert-with-undo (window-key-handler w) *object-template*)
...
(defun add-documentation (theWindow theFile thingsName type)
  (let ((theObject (read-from-string (concatenate 'string "sk8dev::" (pathname-name theFile)))))
    (if (boundp theObject)
      ...
      (error "add-documentation is not implemented for functions, constants or globals."))
...
(defvar *original-fred-function* (symbol-function 'fred))

(let ((*warn-if-redefine* nil)
      (*warn-if-redefine-kernel* nil))
  
  (defun fred (&optional name new-window)
    (if name
      (cond ((string-equal (pathname-type name) "doc")
             (making-doc-window (funcall *original-fred-function* name new-window)))
            ((string-equal (pathname-type name) "sk8")
             (making-sk8script-fred-window (funcall *original-fred-function* name new-window)))
            (t 
             (funcall *original-fred-function* name new-window)))
      (funcall *original-fred-function* name new-window))))
...
#|
	...
	4  	 2/ 7/96	Hernan  	Fixed some bugs preventing opening of some files using the
						doc window style.
	2  	 4/29/96	Hernan  	Fixing insert-object-template.
	2  	 8/ 1/96	Hernan  	Redefining fred to make editing of SK8Script additions take
						effect no matter how you open the window.
	3  	 8/ 1/96	Hernan  	Changed it to not mark the buffer as requiring saving.
	4  	 2/27/97	Hernan  	
